...
class Claim {
  static async create(claimData) {
    ...
    try {
      ...
      await connection.query('UPDATE items SET status = ? WHERE id = ?', ['claimed', item_id]);
  ...
  static async delete(id) {
    ...
    try {
      ...
      await connection.query('UPDATE items SET status = ? WHERE id = ?', ['found', claim.item_id]);
      
      const [result] = await connection.query('DELETE FROM claims WHERE id = ?', [id]);
