...
<h3>Claim System</h3>
<p>Complete security verification workflow for item claims</p>
</td>
</tr>
</table>
...
- ✅ **Claim Verification** - Submit claims with complete security review workflow
- ✅ **Role-Based Access** - Student and security permissions enforced
...
✓ npm package manager
✓ Port 5000available (or modify in .env)
```
...
| <kbd>PUT</kbd> | `/api/items/:id/status` | Update status | 👮 security |
| <kbd>DELETE</kbd> | `/api/items/:id` | Delete item | 👮 security |
...
| <kbd>PUT</kbd> | `/api/claims/:id/verify` | Verify claim | 👮 security |
| <kbd>DELETE</kbd> | `/api/claims/:id` | Cancel claim | 🔐 |

**Legend:** 🔐 = User Auth Required | 👮 = security Only
...
# Authentication
JWT_SECRET="secreatkeyforBack2uapp"
JWT_EXPIRE=7d
...
| Port 5000already in use | Change `PORT` in `.env` or kill process on port 5000|
